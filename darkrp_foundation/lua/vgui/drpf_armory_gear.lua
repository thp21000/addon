...
function PANEL:Init()
	...
	local GearTable = {}
	for k, v in pairs( DarkRPFoundation.CONFIG.ARMORY.Gear ) do
		GearTable[k] = v
		GearTable[k].key = k
	end
	
	table.sort( GearTable, function(a, b) return (a.Level or 0) < (b.Level or 0) end)
	
	local ItemCount = #GearTable
	for k, v in pairs( GearTable ) do
		...
		ItemEquip.DoClick = function()
			if( DarkRPFoundation.CONFIG.ARMORY.Gear[v.key] ) then
				net.Start( "DarkRPFoundationNet_ArmoryEquipGear" )
...
vgui.Register( "drpf_armory_gear", PANEL, "DPanel" )