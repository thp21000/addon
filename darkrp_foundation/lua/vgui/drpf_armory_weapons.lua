...
function PANEL:Init()
	...
	for k, v in pairs( DarkRPFoundation.CONFIG.ARMORY.Weapons ) do
		WeaponsTable[k] = v
		WeaponsTable[k].key = k
	end
	
	table.sort( WeaponsTable, function(a, b) return (a.Level or 0) < (b.Level or 0) end)
	
	local ItemCount = #WeaponsTable
	for k, v in pairs( WeaponsTable ) do
		...
		ItemEquip.DoClick = function()
			if( DarkRPFoundation.CONFIG.ARMORY.Weapons[v.key] ) then
				net.Start( "DarkRPFoundationNet_ArmoryEquipWeapon" )
...
vgui.Register( "drpf_armory_weapons", PANEL, "DPanel" )